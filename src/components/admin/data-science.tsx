...
import { Loader, CalendarIcon, BarChart } from 'lucide-react';
...
export default function DataScience() {
    ...
    const loadReportData = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setData(parsedData);
          const generatedText = generateHardcodedReport(parsedData, selectedDate);
          setReportText(generatedText);
          toast({
            title: 'Laporan Ditemukan',
            description: `Menampilkan laporan untuk tanggal ${format(selectedDate, 'd MMMM yyyy')}.`,
          });
        } else {
          toast({
            variant: 'destructive',
            title: 'Laporan Tidak Ditemukan',
            description: `Tidak ada laporan yang tersimpan untuk tanggal ${format(selectedDate, 'd MMMM yyyy')}.`,
          });
        }
      } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Data Science & Historical Reports</CardTitle>
          <CardDescription>Pilih tanggal untuk melihat laporan penjualan yang tersimpan.</CardDescription>
        </CardHeader>
      ...
      {!isLoading && data.length === 0 && (
         <Card>
            <CardContent className="h-64 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-muted-foreground">Pilih tanggal di atas untuk memuat dan menampilkan data laporan.</p>
            </CardContent>
         </Card>
      )}
    </div>
